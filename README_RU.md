...
- Чистка истории — best-effort: удаляются только адреса, которые
  ...
  независимо от загрузки страницы). Чистка идёт в два прохода: всё, что уже
  известно на момент закрытия вкладки, стирается сразу же, а потом вкладка
  ещё пару секунд слушает события, чтобы поймать то, что могло не успеть
  записаться (например, поисковый запрос, который urlbar ещё не дописал в
  базу). До этого чистка целиком откладывалась на всю паузу, из-за чего
  закрыть вкладку и сразу начать печатать в адресную строку могло на пару
  секунд показать старую подсказку.
- Автозаполнение форм (поисковые поля, любой именованный `<input>`, куда
  вы что-то вводили) тоже чистится. Это отдельная от истории база
  (`formhistory.sqlite`), причём в ней вообще нет понятия контейнера —
  Firefox хранит её одинаково независимо от вкладки/контейнера, поэтому
  для неё понадобилось отдельное отслеживание и отдельная чистка.
- Если приватная вкладка открывает другую вкладку в том же контейнере
  ("Открыть ссылку в новой вкладке", "Duplicate Tab") — контейнер и его
  куки остаются живы, пока не закроются ВСЕ вкладки на нём. Закрытие
  только одной больше не убивает сессию для остальных, ещё открытых.
- Перетаскивание приватной вкладки в НОВОЕ окно безопасно: всё, что она
  успела посетить, чистится сразу, а новое окно подхватывает слежение
  дальше само. Перетаскивание в УЖЕ открытое второе окно так не
  отслеживается — до перезапуска этого окна. Случай достаточно редкий,
  чтобы оставить его как задокументированный пробел, а не городить ради
  него отдельный межоконный механизм.
